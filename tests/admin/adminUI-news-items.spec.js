...
test.describe("News Items", () => {
  ...
  test("markdown preview works in the new news story form", async ({
    page,
  }) => {
    ...
    const precisPreviewText = await page.locator(
      'div.hott-markdown-preview[data-preview-for="#news-item-precis-field"] p',
    );
    await expect(precisPreviewText).toBeVisible();
    await expect(precisPreviewText).toContainText("Chapter: 99");
  });
});
