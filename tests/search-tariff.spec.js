...
test.describe("Find Commodity Page", () => {
  test("Validating the search function with goods name", async ({ page }) => {
    ...
    await expect(
      page.getByRole("definition").filter({ hasText: "White sugar" }),
    ).toBeVisible();
    ...
    const nonPref = page.locator(
      "text=/\\bNon\\s*preferential\\s*tariff\\s*quota\\b/i",
    );
    await expect(
      nonPref.first(),
      "Expected a Non-preferential tariff quota",
    ).toBeVisible();

    //5) Assert order number + 0.00% duty are present in the same row
    const nonPrefRow = page.locator(
      'tr:has-text("Non preferential tariff quota")',
    );
    await expect(
      nonPrefRow.locator("text=/\\b05\\d{4,}\\b/").first(),
    ).toBeVisible(); // order no. like 054321
    await expect(
      nonPrefRow.locator("text=/0\\.00\\s*%/").first(),
    ).toBeVisible(); // duty rate 0.00%
  });
});
