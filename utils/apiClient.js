...
export class ApiClient {
  static URL =
    process.env.FPO_API_URL ??
    "https://search.dev.trade-tariff.service.gov.uk/fpo-code-search";

  /** Retries while API key propagation is delayed (e.g. after create/revoke). */
  static DEFAULT_RETRIES = 120;
  static RETRY_SLEEP_MS = 1000;
  ...
  async handleResponse(opts) {
    ...
    if (success) {
      console.log(
        `Key ${
          opts.expectFailure ? "inactive" : "active"
        } in ${deltaSeconds} seconds`,
      );
    } else {
      console.log(
        `Key failed to be ${
  ...
  assertClassification(expectedClassification) {
    const data = this.json ?? {};
    const results = data?.results ?? [];
    const matchedResult = results.find(
      (result) => result.code === expectedClassification,
    );
    expect(matchedResult).toBeDefined();
  }

  createPostRequestOptions(description) {
